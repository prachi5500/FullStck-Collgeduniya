...
// ✅ STEP 1: CORS SABSE PEHLE (express.json() se pehle)
app.use(cors({
  origin: ['http://localhost:3000', 'http://localhost:3001'],
  credentials: true,
...
// ✅ STEP 3: Request Logger (Debug ke liye)
...
// ✅ POST Route with Detailed Logs
app.post('/colleges', async (req, res) => {
  console.log('📥 POST /colleges - Request received');
  console.log('📦 Body:', req.body);
  
  try {
    const college = new College(req.body);
    const saved = await college.save();
    console.log('✅ College saved with ID:', saved._id);
    res.status(201).json(saved);
  } catch (error) {
...
// ✅ PUT Route
app.put('/colleges/:id', async (req, res) => {
  console.log(`📥 PUT /colleges/${req.params.id}`);
  try {
...
// ✅ DELETE Route
app.delete('/colleges/:id', async (req, res) => {
  console.log(`📥 DELETE /colleges/${req.params.id}`);
  try {
    await College.findByIdAndDelete(req.params.id);
